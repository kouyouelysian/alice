...
available functions:
	...
	bmco_xml_nodeGetByAttributeValue(xmldoc, nodeTag, attributeName, attributeValue)
...
/*  Fetches the first found instance of a tag in an xml document which has
an attribute set to a particular value. Null on fail.
inputs: xmldoc <xml document object> [operational xml object],
		nodeTag <string> [name of the target parent tag]
		attributeName <string> [name of the attribute whose value is being searched for],
		attributeValue <string> [value of attribute to match for]
return: <xml element> or null if not found
*/
function bmco_xml_nodeGetByAttributeValue(xmldoc, nodeTag, attributeName, attributeValue)
{
	var tags = xmldoc.getElementsByTagName(nodeTag);
	for (var t = 0; t < tags.length; t++)
	{
		if (tags[t].getAttribute(attributeName) == attributeValue)
			return tags[t];
	}
	return null;
}
